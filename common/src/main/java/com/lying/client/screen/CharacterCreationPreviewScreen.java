...
public class CharacterCreationPreviewScreen extends CharacterSheetDisplayScreen<CharacterCreationScreenHandler>
{
	...
	public void init()
	{
		...
		int midX = width / 2;
		int midY = height / 2;
		addDrawableChild(ButtonWidget.builder(Text.literal("Back"), button -> mc.setScreen(new CharacterCreationEditScreen(getScreenHandler(), this.inventory, this.title))).dimensions(midX - 55, midY - 125, 50, 20).build());
